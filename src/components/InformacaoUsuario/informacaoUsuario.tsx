  ...
  return (
    <div className="min-h-screen bg-[#ececea] pb-8">
      <main className="mx-auto grid max-w-7xl gap-6 px-7 py-7 lg:grid-cols-[1.35fr_0.85fr]">
        <Card className="border-[#dedad0] bg-[#fdfcf8] shadow-none">
          <CardContent className="p-6">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              {[
                { value: serviceCount, label: "Serviços publicados" },
                { value: skillCount, label: "Habilidades" },
                { value: `${profileProgress}%`, label: "Perfil completo" },
              ].map(({ value, label }) => (
                <div
                  key={label}
        ...
        <Card className="border-[#dedad0] bg-[#fdfcf8] shadow-none">
          <CardContent className="p-6 text-center">
            {usuario?.avatar ? (
              ...
              <div className="w-16 h-16 rounded-full border border-dashed border-[#ccc] bg-[#f9f9f9] mx-auto mb-3 flex items-center justify-center">
                <span className="text-2xl">📷</span>
              </div>
            )}
            <p className="text-sm text-[#888] mb-2">Sem foto de perfil</p>
            <button className="w-full bg-[#f5f5f5] border border-[#ddd] rounded-md py-2 text-sm text-[#555]">
              ⬆️ Enviar foto
            </button>
          </CardContent>
        </Card>
        ...
        <Card className="border-[#dedad0] bg-[#fdfcf8] shadow-none">
          <CardContent className="p-6">
            <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[#aaa] mb-3">
              Complete seu perfil
            </p>
            <div className="flex justify-between mb-2">
              <span className="text-sm text-[#777]">
                {completedFields} de 7 campos preenchidos
              </span>
            ...
            <div className="flex flex-col gap-2 text-sm">
              {[
                { done: true, label: "Nome" },
                { done: true, label: "Status da conta" },
                { done: !!usuario?.bio, label: "Bio" },
                { done: !!usuario?.regiao, label: "Região" },
                { done: !!usuario?.avatar, label: "Foto de perfil" },
                { done: skillCount > 0, label: "Habilidades" },
              ].map(({ done, label }) => (
                <div
                  key={label}
                  className={`flex items-center gap-2 ${done ? "text-[#555]" : "text-[#bbb]"}`}
                >
                  <span
                    className={`${done ? "text-[#1d9e75]" : "text-[#ccc]"} text-base`}
                  >
                    {done ? "✓" : "○"}
                  </span>
                  <span>{label}</span>
